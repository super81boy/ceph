...
TEST_F(TestIOBlockGuard, NonDetainedOps) {
  ...
  ASSERT_EQ(0, op_block_uard.detain({1, 3}, &op1, &cell1));

  Operation op2;
  BlockGuardCell *cell2;
  ASSERT_EQ(0, op_block_uard.detain({0, 1}, &op2, &cell2));

  Operation op3;
  BlockGuardCell *cell3;
  ASSERT_EQ(0, op_block_uard.detain({3, 6}, &op3, &cell3));

  Operations released_ops;
  op_block_uard.release(cell1, &released_ops);
  ASSERT_TRUE(released_ops.empty());

  op_block_uard.release(cell2, &released_ops);
  ASSERT_TRUE(released_ops.empty());

  op_block_uard.release(cell3, &released_ops);
  ASSERT_TRUE(released_ops.empty());
}
...
TEST_F(TestIOBlockGuard, DetainedOps) {
  ...
  Operation op2;
  BlockGuardCell *cell2;
  ASSERT_EQ(1, op_block_uard.detain({2, 6}, &op2, &cell2));
  ASSERT_EQ(nullptr, cell2);

  Operation op3;
  BlockGuardCell *cell3;
  ASSERT_EQ(1, op_block_uard.detain({0, 2}, &op3, &cell3));
